   ...
   if [ -z "${bins-}" ];
    ...
    do
      ...
      then
        args=()
        args+=( "--mode Avg" )
        args+=( "--dist ${output}.dist" )
        args+=( "--years_per_gen ${years_per_gen}" )
        args+=( "-i ${output}" )
        args+=( "-o ${output}" )
        [[ ! -z "${bins-}" ]] && args+=( "--bins $bins" )
        ${PATH_TO_RELATE}/bin/RelateMutationRate ${args[@]}

        args=()
        args+=( "--mrate ${output}_avg.rate" )
      fi

      args+=( "--mode SampleBranchLengths" )
      args+=( "--coal ${output}.coal" )
      args+=( "--dist ${output}.dist" )
      args+=( "--num_samples 1" )
      args+=( "-m ${mu}" )
      args+=( "-i ${output}" )
      ...
      ${PATH_TO_RELATE}/bin/RelateCoalescentRate ${args[@]}
    ...
    #estimate mutation rate and coalescent rates for a final time
      ...
      args+=( "--mode ReEstimateBranchLengths" )
      args+=( "--coal ${output}.coal" )
      args+=( "--dist ${output}.dist" )
      args+=( "-m ${mu}" )
      args+=( "-i ${filename}" )
    ...
    do
      ...
      do
        ...
        args+=( "--mode SampleBranchLengths" )
        args+=( "--coal ${output}.coal" )
        args+=( "--dist ${output}_chr${chr}.dist" )
        args+=( "--num_samples 1" )
        args+=( "-m ${mu}" )
        args+=( "-i ${output}_chr${chr}" )
        args+=( "-o ${output}_chr${chr}" )
        [[ ! -z "${seed-}" ]] && args+=( "--seed $(($seed + $i))" )
  ...
  then
    ...
    do
      ...
      ReEstimateBranchLengths (){
        ...
        args+=( "--num_samples 1" )
        args+=( "-m ${mu}" )
        args+=( "-i ${output}_tmp_chr${chunk}" )
        args+=( "-o ${output}_tmp_chr${chunk}" )
        [[ ! -z "${seed-}" ]] && args+=( "--seed $(($seed + $i))" )
        [[ ! -z "${norm_mutrate-}" ]] && args+=( "--mrate ${output}_avg.rate" )

        ${PATH_TO_RELATE}/bin/RelateCoalescentRate ${args[@]} 2> ${output}_tmp_chr${chunk}.log 
      ...
      then
        ...
        then
          ...
          ${PATH_TO_RELATE}/bin/RelateMutationRate ${args[@]}
            ...
            args+=( "--num_samples 1" )
            args+=( "-m ${mu}" )
            args+=( "-i ${output}_chr${chr}_tmp_chr${chunk}" )
            args+=( "-o ${output}_chr${chr}_tmp_chr${chunk}" )
            [[ ! -z "${seed-}" ]] && args+=( "--seed $(($seed + $i))" )
            [[ ! -z "${norm_mutrate-}" ]] && args+=( "--mrate ${output}_avg.rate" )
