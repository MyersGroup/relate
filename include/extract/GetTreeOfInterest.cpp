...
GetTreeOfInterest(cxxopts::Options& options){
  ...
  bool help = false;
  if(!options.count("anc") || !options.count("mut") || !options.count("bp_of_interest") || !options.count("output")){
    std::cout << "Not enough arguments supplied." << std::endl;
    std::cout << "Needed: anc, mut, bp_of_interest, output." << std::endl;
    help = true;
  }
  ...
  int bp_of_interest = options["bp_of_interest"].as<int>();
  int index_of_bp_of_interest;

  std::cerr << "---------------------------------------------------------" << std::endl;
  std::cerr << "Get tree from " << options["anc"].as<std::string>() << " at BP " << bp_of_interest << "..." << std::endl;
  ...
  index_of_bp_of_interest = -1;
  for(std::vector<SNPInfo>::iterator it_mut = mut.info.begin(); it_mut != mut.info.end(); it_mut++){
    if((*it_mut).pos > bp_of_interest) break;
    index_of_bp_of_interest++;
  }
  if(index_of_bp_of_interest == -1){
