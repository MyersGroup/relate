...
float
GetDirCoalescentRate(Node n, int N, std::vector<float>& epoch, std::vector<CollapsedMatrix<float>>& coalescent_rate_data, CollapsedMatrix<float>& ctime, std::vector<int>& leaves){
  ...
  if(n.child_left != NULL){
    ...
    float coalescent_time = GetDirCoalescentRate(child_left, N, epoch, coalescent_rate_data, ctime, leaves_child_left) + child_left.branch_length;
    GetDirCoalescentRate(child_right, N, epoch, coalescent_rate_data, ctime, leaves_child_right);
    ...
    for(std::vector<int>::iterator it_leaves_child_left1 = leaves_child_left.begin(); it_leaves_child_left1 != leaves_child_left.end(); it_leaves_child_left1++){
      for(std::vector<int>::iterator it_leaves_child_left2 = std::next(it_leaves_child_left1, 1); it_leaves_child_left2 != leaves_child_left.end(); it_leaves_child_left2++){
        for(std::vector<int>::iterator it_leaves_child_right = leaves_child_right.begin(); it_leaves_child_right != leaves_child_right.end(); it_leaves_child_right++){
          ...
          if(coalescent_time < epoch[e+1]){
            coalescent_rate_data[e][0][m] += 1.0;
            coalescent_rate_data[e][1][m] += (coalescent_time - ctime[*it_leaves_child_left1][*it_leaves_child_left2]);
          }else{
            coalescent_rate_data[e][1][m] += (epoch[e+1] - ctime[*it_leaves_child_left1][*it_leaves_child_left2]);
            e++;
            for(; e < (int) epoch.size() - 1; e++){
              if(coalescent_time < epoch[e+1]){
                assert(coalescent_time >= epoch[e]);
                coalescent_rate_data[e][0][m] += 1.0;
                coalescent_rate_data[e][1][m] += (coalescent_time - epoch[e]);
                break;
              }else{
                assert(coalescent_time >= epoch[e+1]);
                coalescent_rate_data[e][1][m] += (epoch[e+1] - epoch[e]);
              }
            } 
          }
      ...
      for(std::vector<int>::iterator it_leaves_child_right1 = leaves_child_right.begin(); it_leaves_child_right1 != leaves_child_right.end(); it_leaves_child_right1++){
        for(std::vector<int>::iterator it_leaves_child_right2 = std::next(it_leaves_child_right1, 1); it_leaves_child_right2 != leaves_child_right.end(); it_leaves_child_right2++){
          ...
          if(coalescent_time < epoch[e+1]){
            coalescent_rate_data[e][0][m] += 1.0;
            coalescent_rate_data[e][1][m] += (coalescent_time - ctime[*it_leaves_child_right1][*it_leaves_child_right2]);
          }else{
            coalescent_rate_data[e][1][m] += (epoch[e+1] - ctime[*it_leaves_child_right1][*it_leaves_child_right2]);
            e++;
            for(; e < (int) epoch.size() - 1; e++){
              if(coalescent_time < epoch[e+1]){
                assert(coalescent_time >= epoch[e]);
...
int CoalescentRateDir(cxxopts::Options& options, int chr = -1){
  ...
  std::vector<CollapsedMatrix<float>> coalescent_rate_data(num_epochs);
  epoch[0] = 0.0;
  coalescent_rate_data[0].resize(2, dim);
  std::fill(coalescent_rate_data[0].vbegin(), coalescent_rate_data[0].vend(), 0.0);
  epoch[1] = 1e3/years_per_gen;
  coalescent_rate_data[1].resize(2, dim);
  std::fill(coalescent_rate_data[1].vbegin(), coalescent_rate_data[1].vend(), 0.0);
  ...
  for(int e = 2; e < num_epochs-1; e++){
    ...
    epoch[e] = std::exp( log_10 * ( 3.0 + 4.0 * (e-1.0)/(num_epochs-3.0) ))/years_per_gen;
    coalescent_rate_data[e].resize(2, dim);
    std::fill(coalescent_rate_data[e].vbegin(), coalescent_rate_data[e].vend(), 0.0);
  ...
  for(; it_anc != std::prev(anc.seq.end(),1); it_anc++){
    ...
    GetDirCoalescentRate(*std::prev((*it_anc).tree.nodes.end(),1), data.N, epoch, coalescent_rate_data, ctime, leaves);
