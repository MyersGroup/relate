...
Data::MakeChunks(const std::string& filename_haps, const std::string& filename_sample, const std::string& filename_map, const std::string& filename_dist, const std::string& file_out, bool use_transitions, float min_memory){
	...
	for(snp = 0; snp < L; snp++){
		...
		std::memcpy(dummy, rsid[snp].c_str(), std::min(rsid[snp].size(), sizeof(dummy)-1));
		fwrite(&dummy[0], sizeof(char), 1024, fp_props);
		std::memset(dummy, 0, sizeof(dummy)); 
		std::memcpy(dummy, ancestral[snp].c_str(), std::min(ancestral[snp].size(), sizeof(dummy)-1));
		fwrite(&dummy[0], sizeof(char), 1024, fp_props);
		std::memset(dummy, 0, sizeof(dummy)); 
		std::memcpy(dummy, alternative[snp].c_str(), std::min(alternative[snp].size(), sizeof(dummy)-1));
		fwrite(&dummy[0], sizeof(char), 1024, fp_props);
	}
