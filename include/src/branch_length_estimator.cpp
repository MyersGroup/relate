...
void
EstimateBranchLengthsWithSampleAge::InitializeOrder2(Tree & tree, bool is_ancient){
	//Randomly shuffle the order of internal nodes.
	//Perhaps the best strategy is to go from root to tips, keep a list of nodes at the current level
	//and randomly choose one, replacing it with the two children. 

	coordinates.resize(N_total);
	std::fill(coordinates.begin(), coordinates.end(), 0.0);
	std::vector<int> active_nodes(N);
	std::vector<double> active_weights(N);
	int num_active_nodes = 1;
	active_nodes[0] = root;
	active_weights[0] = 1.0;
	int internal_node_rank = 1;
	std::uniform_real_distribution<double> dist_active(0.0, 1.0);
	const double child_weight_decay = 0.5;

	while(internal_node_rank < N && num_active_nodes > 0){
		double total_weight = 0.0;
		for(int i = 0; i < num_active_nodes; i++){
			total_weight += active_weights[i];
		}

		double x = dist_active(rng) * total_weight;
		int random_active_node = 0;
		while(random_active_node + 1 < num_active_nodes && x > active_weights[random_active_node]){
			x -= active_weights[random_active_node];
			random_active_node++;
		}

		int node_label = active_nodes[random_active_node];
		coordinates[node_label] = 2.0*(1.0/internal_node_rank - 1.0/N);
		internal_node_rank++;

		double child_weight = active_weights[random_active_node] * child_weight_decay;
		if(child_weight == 0.0) child_weight = std::numeric_limits<double>::min();
		active_nodes[random_active_node] = active_nodes[num_active_nodes-1];
		active_weights[random_active_node] = active_weights[num_active_nodes-1];
		num_active_nodes--;

		int child_label = tree.nodes[node_label].child_left->label;
		if(tree.nodes[child_label].child_left != NULL){
			active_nodes[num_active_nodes] = child_label;
			active_weights[num_active_nodes] = child_weight;
			num_active_nodes++;
		}

		child_label = tree.nodes[node_label].child_right->label;
		if(tree.nodes[child_label].child_left != NULL){
			active_nodes[num_active_nodes] = child_label;
			active_weights[num_active_nodes] = child_weight;
			num_active_nodes++;
	...
	//Now I need to bump up the coordinates of ancient individuals
	if(is_ancient){
		double epsilon = 1e-2;
		for(int i = 0; i < N; i++){
			if(sample_age[i] > 0){
				coordinates[i] = sample_age[i];
				int child = i;
				while(tree.nodes[child].parent != NULL){
					int parent = tree.nodes[child].parent->label;
					double min_parent_age = coordinates[child] + epsilon;
					if(coordinates[parent] >= min_parent_age) break;
					coordinates[parent] = min_parent_age;
					child = parent;
				}
			}
		}
	...
	assert(order[sorted_indices[N_total-1]] == N_total-1);
}

void
EstimateBranchLengthsWithSampleAge::InitializeOrder3(Tree & tree, bool is_ancient){
	...
	std::vector<char> active_lineage(N_total, 0);
	std::vector<char> in_feasible(N_total, 0);
	std::vector<int> feasible_nodes;
	feasible_nodes.reserve(N-1);

	for(int i = 0; i < N; i++){
		active_lineage[i] = 1;
	}

	auto add_if_feasible = [&](int node_label){
		if(node_label >= N && !in_feasible[node_label]){
			int child_left_label  = tree.nodes[node_label].child_left->label;
			int child_right_label = tree.nodes[node_label].child_right->label;
			if(active_lineage[child_left_label] && active_lineage[child_right_label]){
				in_feasible[node_label] = 1;
				feasible_nodes.push_back(node_label);
			}
		}
	};

	for(int node_label = N; node_label < N_total; node_label++){
		add_if_feasible(node_label);
	}

	std::uniform_int_distribution<int> dist_feasible;
	for(int event_rank = 1; event_rank < N; event_rank++){
		assert(!feasible_nodes.empty());

		int feasible_index = dist_feasible(
				rng,
				std::uniform_int_distribution<int>::param_type(0, (int) feasible_nodes.size()-1)
				);
		int node_label = feasible_nodes[feasible_index];
		feasible_nodes[feasible_index] = feasible_nodes[feasible_nodes.size()-1];
		feasible_nodes.pop_back();
		in_feasible[node_label] = 0;

		coordinates[node_label] = 2.0*(1.0/(N-event_rank) - 1.0/N);
		active_lineage[node_label] = 1;

		if(tree.nodes[node_label].parent != NULL){
			add_if_feasible(tree.nodes[node_label].parent->label);
		}
	}
...
EstimateBranchLengthsWithSampleAge::RandomSwitchOrder(Tree& tree, int node_k, std::uniform_real_distribution<double>& dist_unif){
	...
	if(node_k >= N){
		...
		if( parent_order - child_order > 2 ){
			...
			static thread_local std::uniform_int_distribution<int> d_swap;
			int new_order = d_swap(rng, std::uniform_int_distribution<int>::param_type(child_order+1, parent_order-1)); 
		...
		if(tau_below >= 0 && tau_above >= 0){
			...
			if(tau_new_above >= 0.0 && tau_new_below >= 0.0){
				...
				if(0){
					...
					//obtain order of coalescent events
					for (int pos = 0; pos < N_total; ++pos) {
						order_new2[sorted_indices_new2[pos]] = pos;
					}
		...
		if(tau_above >= 0.0 || tau_below >= 0.0){
			...
			if(tau_new_above >= 0.0 || tau_new_below >= 0.0){
				...
				if(1){
					...
					//obtain order of coalescent events
					for (int pos = 0; pos < N_total; ++pos) {
						order_new[sorted_indices_new[pos]] = pos;
					}
...
EstimateBranchLengthsWithSampleAge::ChangeTimeWhilekAncestors_new(Tree& tree, int node, std::uniform_real_distribution<double>& dist_unif){
	...
	double bl;
	for(std::vector<int>::iterator it_s = std::next(sorted_indices.begin(), k); it_s != sorted_indices.end(); it_s++){
		if(*it_s < N){
			bl = tree.nodes[*it_s].branch_length;
	...
	if(min_tip > 0 && tau_old > 0){
		...
		log_likelihood = CalculatePrior(k, order.size()-1, coordinates_new, sorted_indices_new, num_lineages_new);
		if(log_likelihood != -std::numeric_limits<float>::infinity()){
			log_likelihood_ratio += log_likelihood;
			log_likelihood = CalculatePrior(k, order.size()-1, coordinates, sorted_indices, num_lineages);
			if(log_likelihood != -std::numeric_limits<float>::infinity()) log_likelihood_ratio -= log_likelihood;
		}
...
EstimateBranchLengthsWithSampleAge::ChangeTimeWhilekAncestorsVP_new(Tree& tree, int node, const std::vector<double>& epoch, const std::vector<double>& coal_rate, std::uniform_real_distribution<double>& dist_unif){
	...
	if(min_tip > 0 && tau_old > 0.0){
		...
		log_likelihood = CalculatePrior(k, order.size()-1, epoch, coal_rate, coordinates_new, sorted_indices_new, num_lineages_new);
		//log_likelihood = CalculatePrior(epoch, coal_rate, coordinates_new, sorted_indices_new, num_lineages_new);
		if(log_likelihood != -std::numeric_limits<float>::infinity()){
			//tmp3 = log_likelihood;
			log_likelihood_ratio += log_likelihood;
			log_likelihood = CalculatePrior(k, order.size()-1, epoch, coal_rate, coordinates, sorted_indices, num_lineages);
			//log_likelihood = CalculatePrior(epoch, coal_rate, coordinates, sorted_indices, num_lineages);
			//std::cerr << tmp1 - tmp2 << " " << tmp3 - log_likelihood << std::endl;
			if(log_likelihood != -std::numeric_limits<float>::infinity()) log_likelihood_ratio -= log_likelihood;
		}
...
EstimateBranchLengthsWithSampleAge::MCMC(const Data& data, Tree& tree, const int seed){
	...
	if(1){
		InitializeOrder3(tree, is_ancient);
		//for(int j = 0; j < (int) 10*data.N; j++){
	...
	for(int m = 0; m < 200; m++){
		for(int j = 2*data.N-2; j >= data.N; j--){
			UpdateOneEvent(tree, j, dist_gamma, dist_unif);
		}
	}
...
EstimateBranchLengthsWithSampleAge::MCMCVariablePopulationSize(const Data& data, Tree& tree, const std::vector<double>& epoch, std::vector<double>& coal_rate, const int seed){
	...
	if(total_bl == 0){
		...
		for(int m = 0; m < 200; m++){
			for(int j = 2*data.N-2; j >= data.N; j--){
				UpdateOneEventVP(tree, j, epoch, coal_rate, dist_gamma, dist_unif);
			}
		}
...
////////////
void
EstimateBranchLengthsWithSampleAge::MCMCVariablePopulationSizeForRelateOld(const Data& data, Tree& tree, const std::vector<double>& epoch, std::vector<double>& coal_rate, const int seed){
...
////////////
void
EstimateBranchLengthsWithSampleAge::MCMCVariablePopulationSizeForRelateNew(const Data& data, Tree& tree, const std::vector<double>& epoch, std::vector<double>& coal_rate, const int seed){
