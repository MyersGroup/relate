...
AncesTree::ReadTreesimOldStyle(const std::string& filename, const Data& data){
  ...
  for(int i = 0; i < 7; i++){
    ...
    if(i == 2){
      ...
      std::string N_str;
      readN >> N_str >> N_str >> N_str >> N_str;
      N = stoi(N_str);
    }
  }
  int N_total = 2*N-1;
  int L = 1;
  //need to fill in parents and children
  seq.emplace_back();
  CorrTrees::iterator it_seq = seq.begin();

  (*it_seq).tree.nodes.resize(N_total);
  int child1, child2, parent = 0;
  int old_site = 0, site;
  float time;
  ...
  while(std::getline(is, line)){
    ...
    std::istringstream oldStyle(line); 
    oldStyle >> site >> child1 >> child2 >> time;

    //if site changes I need to reset parent
    if(site != old_site){
      //determine branch lengths
      for(int n = N; n < N_total; n++){
        (*((*it_seq).tree.nodes[n].child_left)).branch_length  = coordinates[n] - coordinates[(*((*it_seq).tree.nodes[n].child_left)).label];
        (*((*it_seq).tree.nodes[n].child_right)).branch_length = coordinates[n] - coordinates[(*((*it_seq).tree.nodes[n].child_right)).label];       
      }
      (*it_seq).pos = data.pos[L-1];
      old_site = site;
      parent = 0;  
      L++;
      seq.emplace_back();
    ...
    if(parent < 2*N-1){
      if(child1 != -1 && child2 != -1){
        child1--;
        child2--;  
        (*it_seq).tree.nodes[child1].parent      = &(*it_seq).tree.nodes[parent];
        (*it_seq).tree.nodes[child2].parent      = &(*it_seq).tree.nodes[parent];
        (*it_seq).tree.nodes[parent].child_left  = &(*it_seq).tree.nodes[child1];
        (*it_seq).tree.nodes[parent].child_right = &(*it_seq).tree.nodes[child2]; 
      }
      (*it_seq).tree.nodes[parent].label = parent;
      coordinates[parent]               = time;
    }
    parent++;
